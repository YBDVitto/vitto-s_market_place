...
router.post('/edit-product', isAuth, (req, res, next) => {
    upload.single('image')(req, res, (err) => {
        if (err) {
            return res.status(400).json({ errors: [{ msg: err.message }] });
        }
...
router.get('/edit-product', isAuth, withAuth(getEditProduct));
router.get('/delete-product', isAuth, withAuth(getDeleteProduct));
router.post('/delete-product', isAuth, withAuth(postDeleteProduct));
